...
{
    import_array();
    if (PyArray_ImportNumPyAPI() < 0) return nullptr;

    PyObject *const mod = PyModule_Create(&cdust_module);
    if (!mod) goto failed;

    if (PyModule_AddType(mod, &pydust_geoid_type)) goto failed;
    if (PyModule_AddType(mod, &pydust_line_type)) goto failed;
    if (PyModule_AddType(mod, &pydust_surface_type)) goto failed;

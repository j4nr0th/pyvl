 ...
 */
static bool ensure_fmm_and_state(PyTypeObject *defining_class, PyObject *self, const PyVL_FMMTreeObject **p_this,
                                 const module_state_t **p_state)
{
    ...
    if (!PyObject_TypeCheck(self, state->fmm_tree_type))
    {
        PyErr_Format(PyExc_TypeError, "Self was not \"%s\" but was \"%s\".", state->fmm_tree_type->tp_name,
                     Py_TYPE(self)->tp_name);
        return false;
    }
    *p_this = (const PyVL_FMMTreeObject *)self;
    *p_state = state;
    return true;
}
 ...
 */
static PyArrayObject *fmm_flatten_points(PyObject *obj, const char *arg_name, size_t *p_n)
{
    PyArrayObject *arr =
...
PyDoc_STRVAR(pyvl_fmm_tree_type_docstring,
             "FMMTree(order=4, critical_particle_count=4, max_depth=20, work_order=None, "
             "alpha_centroid=0.5)\n"
             "Fast Multipole Method octree for O(N) far-field induction from vortex particles.\n"
             "\n"
             "The tree partitions vortex sources into an octree and uses\n"
             "multipole-to-local (M2L) conversion to achieve linear-time\n"
             "evaluation.  Unlike Barnes-Hut (:class:`BarnesHutTree`) which\n"
             "evaluates each well-separated cell's multipole individually\n"
             "(:math:`O(N \\log N)`), the FMM pre-converts all V-list multipoles\n"
             "into a single local expansion per leaf during the build phase,\n"
             "making eval :math:`O(N)`.\n"
             "\n"
             ...
             "See Also\n"
             "--------\n"
             "FMMTree.build : Construct and populate in one step.\n"
             "BarnesHutTree : :math:`O(N \\log N)` tree-code alternative.\n"
             "\n"
             "Examples\n"
             "--------\n"
             ">>> from pyvl.cvl import FMMTree\n"
...
static PyObject *pyvl_fmm_tree_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    PyVL_FMMTreeObject *self = (PyVL_FMMTreeObject *)type->tp_alloc(type, 0);
    if (!self)
        return NULL;

    // Zero-initialise everything.
    self->tree = (fmm_tree_t){0};
    self->n_threads = 1;
    ...
    self->tree.settings = (fmm_settings_t){
...
static void pyvl_fmm_tree_dealloc(PyObject *self)
{
    PyObject_GC_UnTrack(self);
    PyVL_FMMTreeObject *this = (PyVL_FMMTreeObject *)self;
...
static int pyvl_fmm_tree_traverse(PyObject *self, visitproc visit, void *arg)
{
    PyVL_FMMTreeObject *this = (PyVL_FMMTreeObject *)self;
...
static PyObject *pyvl_fmm_tree_str(PyObject *self)
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    if (this->built)
    {
        char buf[160];
        snprintf(buf, sizeof(buf),
                 "FMMTree(order=%u, n_sources=%u, n_nodes=%u, "
    ...
    {
        char buf[80];
        snprintf(buf, sizeof(buf), "FMMTree(order=%u, unbuilt)", this->tree.settings.order);
        return PyUnicode_FromString(buf);
    }
}

static PyObject *pyvl_fmm_tree_repr(PyObject *self)
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    if (this->built)
    {
        char buf[200];
        snprintf(buf, sizeof(buf), "<FMMTree order=%u n_sources=%u n_nodes=%u depth=%u at %p>",
                 this->tree.settings.order, this->tree.n_sources, this->tree.n_nodes, this->tree.max_depth_reached,
                 (void *)self);
        return PyUnicode_FromString(buf);
    }
    ...
    {
        char buf[120];
        snprintf(buf, sizeof(buf), "<FMMTree order=%u unbuilt at %p>", this->tree.settings.order, (void *)self);
        return PyUnicode_FromString(buf);
    }
}
...
static PyObject *pyvl_fmm_tree_get_n_sources(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_sources);
}

static PyObject *pyvl_fmm_tree_get_n_nodes(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_nodes);
}

static PyObject *pyvl_fmm_tree_get_n_internal(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_internal);
}

static PyObject *pyvl_fmm_tree_get_n_multipole_leaves(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_multipole_leaves);
}

static PyObject *pyvl_fmm_tree_get_n_particle_leaves(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_particle_leaves);
}

static PyObject *pyvl_fmm_tree_get_max_depth(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.max_depth_reached);
}

static PyObject *pyvl_fmm_tree_get_memory_bytes(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong((unsigned long)this->tree.buffer_size);
}

static PyObject *pyvl_fmm_tree_get_order(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.settings.order);
}

static PyObject *pyvl_fmm_tree_get_alpha_centroid(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyFloat_FromDouble((double)this->tree.settings.alpha_centroid);
}

static PyObject *pyvl_fmm_tree_get_critical_particle_count(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.settings.critical_particle_count);
}

static PyObject *pyvl_fmm_tree_get_max_depth_setting(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.settings.max_depth);
}

static PyObject *pyvl_fmm_tree_get_n_leaves(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong(this->tree.n_leaves);
}

static PyObject *pyvl_fmm_tree_get_vlist_count(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong((unsigned long)this->tree.vlist_count);
}

static PyObject *pyvl_fmm_tree_get_nflist_count(PyObject *self, void *Py_UNUSED(closure))
{
    const PyVL_FMMTreeObject *this = (const PyVL_FMMTreeObject *)self;
    return PyLong_FromUnsignedLong((unsigned long)this->tree.nflist_count);
}

static PyGetSetDef pyvl_fmm_tree_getset[] = {
    {
        .name = "n_sources",
        .get = pyvl_fmm_tree_get_n_sources,
        .doc = "int : Number of source particles.",
    },
    {
        .name = "n_nodes",
        .get = pyvl_fmm_tree_get_n_nodes,
        .doc = "int : Total number of octree nodes.",
    },
    {
        .name = "n_internal",
        .get = pyvl_fmm_tree_get_n_internal,
        .doc = "int : Number of internal (non-leaf) nodes.",
    },
    {
        .name = "n_multipole_leaves",
        .get = pyvl_fmm_tree_get_n_multipole_leaves,
        .doc = "int : Number of multipole-bearing leaves.",
    },
    {
        .name = "n_particle_leaves",
        .get = pyvl_fmm_tree_get_n_particle_leaves,
        .doc = "int : Number of uncompressed particle leaves.",
    },
    {
        .name = "max_depth",
        .get = pyvl_fmm_tree_get_max_depth,
        .doc = "int : Maximum depth actually reached in the tree.",
    },
    {
        .name = "memory_bytes",
        .get = pyvl_fmm_tree_get_memory_bytes,
        .doc = "int : Size of the tree buffer in bytes.",
    },
    {
        .name = "order",
        .get = pyvl_fmm_tree_get_order,
        .doc = "int : Multipole order used by the tree.",
    },
    {
        .name = "alpha_centroid",
        .get = pyvl_fmm_tree_get_alpha_centroid,
        .doc = "float : Centroid-based subdivision threshold (0.0 = disabled).",
    },
    {
        .name = "critical_particle_count",
        .get = pyvl_fmm_tree_get_critical_particle_count,
        .doc = "int : Base subdivision threshold.",
    },
    {
        .name = "max_depth_setting",
        .get = pyvl_fmm_tree_get_max_depth_setting,
        .doc = "int : Maximum depth cap configured at build time.",
    },
    {
        .name = "n_leaves",
        .get = pyvl_fmm_tree_get_n_leaves,
        .doc = "int : Number of leaves in the tree.",
    },
    {
        .name = "vlist_count",
        .get = pyvl_fmm_tree_get_vlist_count,
        .doc = "int : Total V-list (well-separated) entries across all leaves.",
    },
    {
        .name = "nflist_count",
        .get = pyvl_fmm_tree_get_nflist_count,
        .doc = "int : Total near-field list entries across all leaves.",
    },
    {0},
};
 ...
 *        alpha_centroid=0.5, n_threads=1)
 * ----------------------------------------------------------------
 */

PyDoc_STRVAR(pyvl_fmm_tree_build_doc, "build(sources_coords, sources_values, /, order=4, critical_particle_count=4, "
                                      "max_depth=20, work_order=None, alpha_centroid=0.5, n_threads=1)\n"
                                      "Build an FMM tree from source arrays and return a new tree.\n"
                                      "\n"
                                      "Parameters\n"
                                      "----------\n"
...
static PyObject *pyvl_fmm_tree_build(PyTypeObject *type, PyObject *const *args, const Py_ssize_t nargs,
                                     const PyObject *kwnames)
{
    ...
    PyArrayObject *coords_arr = fmm_flatten_points(coords_obj, "sources_coords", &n_coords);
    if (!coords_arr)
        return NULL;
    PyArrayObject *values_arr = fmm_flatten_points(values_obj, "sources_values", &n_vals);
    if (!values_arr)
    {
    ...
    const bool ok =
        fmm_tree_build((unsigned)n_coords, (unsigned)n_threads, (const real3_t *)PyArray_DATA(coords_arr),
    ...
    if (!ok)
    {
        ...
        Py_DECREF(coords_arr);
        Py_DECREF(values_arr);
        PyErr_SetString(PyExc_RuntimeError, "fmm_tree_build failed.");
        return NULL;
    }
...
/*
 * ----------------------------------------------------------------
 *  eval(self, targets, /, *, theta=0.0, mode="tree_code", n_threads=None, out=None)
 * ----------------------------------------------------------------
 */
...
{
    PyVL_FMMTreeObject *this;
    const module_state_t *state;
    if (!ensure_fmm_and_state(defining_class, self, (const PyVL_FMMTreeObject **)&this, &state))
        return NULL;

    if (!this->built)
    {
        PyErr_SetString(PyExc_RuntimeError, "FMMTree has not been built. Call build() first.");
        return NULL;
    }
    ...
    // Resolve the mode
    fmm_eval_mode_t eval_mode = FMM_EVAL_TREE_CODE;
    if (mode_obj != NULL && !Py_IsNone(mode_obj))
    {
        if (!PyUnicode_Check(mode_obj))
        {
            PyErr_SetString(PyExc_TypeError, "mode must be a string ('tree_code' or 'fmm').");
            return NULL;
        }
        ...
        {
            eval_mode = FMM_EVAL_TREE_CODE;
        }
        else if (strcmp(mode_str, "fmm") == 0)
        {
    ...
    size_t n_targets;
    PyArrayObject *targets_arr = fmm_flatten_points(targets_obj, "targets", &n_targets);
    if (!targets_arr)
        return NULL;
    ...
    fmm_tree_eval_all(&this->tree, src_coords, src_values, (unsigned)n_targets, targets_data, out_data, eval_cfg, nth);
...
PyDoc_STRVAR(pyvl_fmm_tree_multipole_eval_cost_doc, "multipole_eval_cost(order)\n"
...
static PyObject *pyvl_fmm_tree_multipole_eval_cost(PyObject *Py_UNUSED(self), PyObject *const *args,
                                                   const Py_ssize_t nargs, const PyObject *kwnames)
{
    Py_ssize_t order;
    if (parse_arguments_check(
...
PyDoc_STRVAR(pyvl_fmm_tree_direct_sum_cost_doc, "direct_sum_cost(n_sources)\n"
...
static PyObject *pyvl_fmm_tree_direct_sum_cost(PyObject *Py_UNUSED(self), PyObject *const *args, const Py_ssize_t nargs,
                                               const PyObject *kwnames)
{
    Py_ssize_t n;
    if (parse_arguments_check(
...
PyDoc_STRVAR(pyvl_fmm_tree_crossover_order_doc,
...
static PyObject *pyvl_fmm_tree_crossover_order(PyObject *Py_UNUSED(self), PyObject *const *args, const Py_ssize_t nargs,
                                               const PyObject *kwnames)
{
    Py_ssize_t n;
    if (parse_arguments_check(
...
PyDoc_STRVAR(pyvl_fmm_tree_min_sources_for_order_doc,
...
static PyObject *pyvl_fmm_tree_min_sources_for_order(PyObject *Py_UNUSED(self), PyObject *const *args,
                                                     const Py_ssize_t nargs, const PyObject *kwnames)
{
    Py_ssize_t order;
    if (parse_arguments_check(
...
static PyMethodDef pyvl_fmm_tree_methods[] = {
    {
        .ml_name = "build",
        .ml_meth = (void *)pyvl_fmm_tree_build,
        .ml_flags = METH_CLASS | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_build_doc,
    },
    {
        .ml_name = "eval",
        .ml_meth = (void *)pyvl_fmm_tree_eval,
        .ml_flags = METH_METHOD | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_eval_doc,
    },
    {
        .ml_name = "multipole_eval_cost",
        .ml_meth = (void *)pyvl_fmm_tree_multipole_eval_cost,
        .ml_flags = METH_STATIC | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_multipole_eval_cost_doc,
    },
    {
        .ml_name = "direct_sum_cost",
        .ml_meth = (void *)pyvl_fmm_tree_direct_sum_cost,
        .ml_flags = METH_STATIC | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_direct_sum_cost_doc,
    },
    {
        .ml_name = "crossover_order",
        .ml_meth = (void *)pyvl_fmm_tree_crossover_order,
        .ml_flags = METH_STATIC | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_crossover_order_doc,
    },
    {
        .ml_name = "min_sources_for_order",
        .ml_meth = (void *)pyvl_fmm_tree_min_sources_for_order,
        .ml_flags = METH_STATIC | METH_KEYWORDS | METH_FASTCALL,
        .ml_doc = pyvl_fmm_tree_min_sources_for_order_doc,
    },
    {0},
};

PyType_Spec pyvl_fmm_tree_typespec = {
    .name = PYVL_CTYPE_NAME(FMMTree),
    .basicsize = sizeof(PyVL_FMMTreeObject),
    .itemsize = 0,
    .flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_HEAPTYPE,
    .slots =
        (PyType_Slot[]){
            {Py_tp_str, pyvl_fmm_tree_str},
            {Py_tp_repr, pyvl_fmm_tree_repr},
            {Py_tp_doc, (void *)pyvl_fmm_tree_type_docstring},
            {Py_tp_methods, pyvl_fmm_tree_methods},
            {Py_tp_getset, pyvl_fmm_tree_getset},
            {Py_tp_new, pyvl_fmm_tree_new},
            {Py_tp_dealloc, pyvl_fmm_tree_dealloc},
            {Py_tp_traverse, pyvl_fmm_tree_traverse},
            {0},
        },
};

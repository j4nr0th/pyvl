...
@final
class FMMTree:
    r"""Fast Multipole Method octree for O(N) far-field induction from vortex particles.

    The tree partitions vortex sources into an octree and uses
    multipole-to-local (M2L) conversion to achieve linear-time
    evaluation.  Unlike :class:`BarnesHutTree` which evaluates each
    well-separated cell's multipole individually (:math:`O(N \log N)`),
    the FMM pre-converts all V-list multipoles into a single local
    expansion per leaf during the build phase, making eval :math:`O(N)`.
    ...
    alpha_centroid : float, default 0.5
        Centroid-based subdivision threshold.

    See Also
    --------
    FMMTree.build : Construct and populate in one step.
    BarnesHutTree : O(N log N) tree-code alternative.
    ...
    @property
    def vlist_count(self) -> int:
        """Total V-list (well-separated) entries across all leaves."""
        ...

    @property
    def nflist_count(self) -> int:
        """Total near-field list entries across all leaves."""
        ...
    ...
    def eval(
        ...
        *,
        theta: float = 0.0,
        mode: Literal["tree_code", "fmm"] = "tree_code",
        n_threads: int | None = None,
        out: npt.NDArray[np.double] | None = None,
    ) -> npt.NDArray[np.double]:
        """Evaluate the tree at one or more target points.
        ...
        theta : float, default 0.0
            Multipole acceptance criterion.  Only used in ``"tree_code"`` mode.
